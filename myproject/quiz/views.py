...
#        logger.debug("after try-except")
        if q.is_cat15:
            xml = render_to_string('cat_template.xml', {'time':q.time_limit,'mark':q.mark,'penalty':q.penalty,'query_set': q.get_questions().order_by('rank'),'section2_present':q.has_section2})
        else :
